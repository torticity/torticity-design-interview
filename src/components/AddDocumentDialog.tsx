    ...
    <Dialog open={open} onOpenChange={(v) => { if (!v) reset(); onOpenChange(v) }}>
      <DialogContent className="max-w-xl text-base">
        <form onSubmit={handleSubmit}>
          ...
          {/* File upload area */}
          <div className="py-4">
            <Label className="text-base mb-1.5 block">Upload file</Label>
            {file ? (
              <div className="flex items-center gap-3 border rounded-md px-3 py-3 bg-muted/40">
                <FileText className="h-6 w-6 text-muted-foreground shrink-0" />
                <div className="flex-1 min-w-0">
                  <div className="text-base font-medium truncate">{file.name}</div>
                  <div className="text-base text-muted-foreground">{fmtSize(file.size)}</div>
                </div>
                <Button
                  type="button"
                  variant="ghost"
              ...
              <button
                ...
                className={`w-full border-2 border-dashed rounded-md py-8 px-4 flex flex-col items-center gap-2 transition-colors ${
                  dragOver
              ...
              >
                ...
                <div className="text-base text-muted-foreground">PDF, JPG, PNG, or TXT</div>
              </button>
            )}
            <input
              ref={inputRef}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              <Label className="text-base mb-1.5 block">Document Name</Label>
              <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Lab Results - CBC 12/01/25" />
            </div>
            <div className="col-span-1">
              <Label className="text-base mb-1.5 block">Status</Label>
            ...
            <div className="col-span-2 flex items-center gap-2 pt-1">
              <input
                ...
                className="h-4 w-4"
              />
              <Label htmlFor="encrypted" className="text-base cursor-pointer">Encrypted</Label>
            </div>
          </div>
